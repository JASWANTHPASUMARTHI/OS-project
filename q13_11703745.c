...
void wait2()
{
	...
	{
		p1[q2[i]-1].wait_time+=1;			
	}

int process_arrival(int time)
{ int i,j;
	int *id=(int*)malloc(sizeof(int)*process_count);	
	int pos=-1;
	...
	else							
	{
		...
		for(i=1;i<=pos;i++)
		{
			if(p1[id[i]-1].priority<p1[max_p-1].priority)		
			{
				max_p=id[i];
			}
		}
...
int allocate_p()
{
	...
	else
	{
		...
		for(i=1;i<=queue1_size;i++)
		{
			...
			else if(p1[q1[i]-1].priority==p1[max_p-1].priority)		
			{
...
int preemptive()
{
	...
	{
		new_id=process_arrival(time);			
		if(new_id!=0)
		{
			if(run_id!=0)
			{
				if(p1[new_id-1].priority<p1[run_id-1].priority)		
				{
					delete1(run_id);
					insert2(run_id);		
